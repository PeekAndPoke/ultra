...
class MpLocalDateRangeSpec : StringSpec({
    ...
    "Comparing must work with direct comparison operators" {
        ...
        (range(3) < twoDaysPeriod) shouldBe false

        (range(1) <= twoDaysPeriod) shouldBe true
        (range(2) <= twoDaysPeriod) shouldBe true
        (range(3) <= twoDaysPeriod) shouldBe false

        (range(1).compareTo(twoDaysPeriod) == 0) shouldBe false
        (range(2).compareTo(twoDaysPeriod) == 0) shouldBe true
        (range(3).compareTo(twoDaysPeriod) == 0) shouldBe false

        (range(1) >= twoDaysPeriod) shouldBe false
        (range(2) >= twoDaysPeriod) shouldBe true
        (range(3) >= twoDaysPeriod) shouldBe true
