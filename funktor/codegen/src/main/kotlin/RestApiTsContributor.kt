...
class RestApiTsContributor(
    ...
    private fun selectedOf(feature: ApiFeature, group: ApiRoutes, route: ApiRoute<*>): Selected {
        ...
        // Only `Plain` is implemented so far. The others are REJECTED rather than skipped: silently
        // emitting a client that is missing half its endpoints is the wrong-and-quiet failure this
        // whole module exists to remove, and a frontend would only notice at the call site.
        check(route is ApiRoute.Plain<*>) {
            "Route '${route.method.value} ${route.pattern.pattern}' (${feature.codeGenName} / " +
                    ...
                    "TypeScript generator does not support yet — only parameterless routes are " +
                    "implemented. See .claude/tasks/20260730-funktor-codegen-rest-contributor.md."
        }
