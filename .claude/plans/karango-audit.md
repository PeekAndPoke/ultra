...
Karango is a type-safe Kotlin DSL for ArangoDB with 47 source files, 131 test files (E2E, requiring ArangoDB), and 20 TODOs. The code is generally clean but lacks KDoc on public APIs and has some dead code. Tests are strong on AQL functions (85+ covered) but weak on infrastructure (driver, codec, indexes, repository methods).
